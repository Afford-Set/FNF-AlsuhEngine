...
class SoundFrontEnd
{
	...
	function update(elapsed:Float):Void
	{
		...
		#if FLX_KEYBOARD
		if (!TitleState.blockedFuckingVolume)
		{
			if (muteKeys.length > 0 && FlxG.keys.anyJustReleased(muteKeys))
				toggleMuted();
			else if (volumeUpKeys.length > 0 && FlxG.keys.anyJustReleased(volumeUpKeys))
				changeVolume(0.1);
			else if (volumeDownKeys.length > 0 && FlxG.keys.anyJustReleased(volumeDownKeys))
				changeVolume(-0.1);
		}
		#end
	}
